...
std::vector<vector<float>> CppClass::getCsv() {
    return csvFile;
}

int CppClass::run(std::string directorio){
    ...
    //stringstream ss;
    //string stringDirectorio;
    //ss << directorio;
    //ss >> stringDirectorio;
    
    //cout << stringDirectorio << endl;
    
    // ARREGLAR!!!
    //if (argc <= 2) {
    if(directorio.compare("") == 0){
        cout << "ERROR: Ningun archivo especificado" << endl;
        return 0;
        //}else if (argc >=2){
    }else{
        /*
    ...
    audioLoad->output("md5").set(m5);
    audioLoad->output("codec").set(codec);
    audioLoad->output("bit_rate").set(bitRate);
    audioLoad->compute();
    delete audioLoad;
    ...
    string poly = "poly.notes.csv";
    //int len=poly.size();
    //string csvFilename = audioFilename.string::substr(0,len-4)+".notes.csv";
    string csvFilename = poly;
    cout << csvFilename << endl;
    ofstream outFile;
    //outFile.open(csvFilename.c_str());
    //( "test\\" + variable + ".txt" ).c_str()
    string prueba = "poly.csv";
    outFile.open( prueba.c_str() );
    if (outFile.fail())
    ...
    // WRITE csvFile //
    csvFile.clear();
    csvFile.push_back(onset);
    csvFile.push_back(duration);
    csvFile.push_back(MIDInote);
